...
st.set_page_config(page_title="Kripto Dip & Hacim Tarayıcı (KuCoin)", layout="centered")
st.title("📉 KuCoin Kripto Dip & Hacim Taraması")
...
def fetch_symbols():
    ...
    return sorted([symbol for symbol in markets if symbol.endswith('/USDT') and markets[symbol]['spot']])

def fetch_ohlcv(symbol, timeframe='1d', since_days=30):  # 30 günle başla
...
def scan_cryptos(symbols, ma_tolerance, volume_threshold, use_ma, use_rsi, rsi_threshold):
    ...
    for symbol in symbols:
        try:
            data = fetch_ohlcv(symbol, since_days=30)  # 30 günle sınırla
            if len(data) < 30:
                continue
...
if st.button("🔍 Kripto Tarama Başlat"):
    with st.spinner("KuCoin üzerindeki kriptolar taranıyor..."):
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                data_plot = fetch_ohlcv(row['Sembol'], since_days=30)
